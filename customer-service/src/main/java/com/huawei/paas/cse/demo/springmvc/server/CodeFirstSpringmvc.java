...
public class CodeFirstSpringmvc {
    ...
    private MetricsListener metrics = new MetricsListener();
    ...
    public String metrics() {
        return metrics.getLatestMetrics();
    }

    @RequestMapping(path = "/notRetry", method = RequestMethod.GET)
    @ApiResponse(code = 400, message = "parameter not valid.", response = String.class)
    public String notRetry(@RequestParam(name = "counter") int counter) {
        LOGGER.info("not Retry " + counter);
        throw new InvocationException(400, "parameter not valid phase.", "parameter not valid object.");
    }
}
